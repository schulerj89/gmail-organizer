...
- Dashboard: first-run tutorial overlay with local browser completion/skipped state, restart tutorial control, left-nav cleanup workbench, quick cleanup queues, sender-group cleanup, AI suggestions queue, contained email queue, email detail dialog with review decision controls, contextual bulk action bar, modal preview/confirmation flow, saved category counts, user-friendly Gmail date search presets with custom before/after calendar dates, manual refresh, find-emails scan, monitoring toggle, AI category suggestions, manual category moves, bulk mark-read, unsubscribe preparation, and move-to-trash.
